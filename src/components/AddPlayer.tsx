...
export default function DisplayTeams(props: AddPlayerProps) {    
    ...
    return (         
        ...
        <div>
            ...
            <textarea rows={2} name="player-info" placeholder={`Player ${props.playerNum} info`} id={`player-${props.playerNum}-info`} defaultValue={props.player[`player-${props.playerNum}-info`]} onKeyUp={(e) => {props.handleTeam(e, 0)}}/>
        </div>
    </div>
    )
};